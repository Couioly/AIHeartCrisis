...
from sqlalchemy import DECIMAL, Enum, String, ForeignKey
...
class UserHealth(Base):
    ...
    username: Mapped[str] = mapped_column(String(50), ForeignKey("user.username"), index=True, comment="关联用户名")
...
async def create_user_health(db, data):
    db_user = UserHealth(**data.model_dump())
    db.add(db_user)
    await db.commit()
